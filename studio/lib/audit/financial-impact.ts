...
export function getFinancialImpact(score: number, lang: FinancialImpactLang = 'ko'): FinancialImpact {
	...
	if (n < 65) {
		...
		return {
			...
			amount,
			mainText: ko
				? `월 약 ${formatted}원 기회 손실 *(시뮬레이션 추정치)`
				: `About ₩${formatted} in monthly opportunity loss *(simulation estimate)`,
			subText: ko
	...
	if (n < 90) {
		...
		return {
			...
			amount,
			mainText: ko
				? `월 약 ${formatted}원 광고 대체 가치 *(시뮬레이션 추정치)`
				: `About ₩${formatted}/mo in ad-equivalent value *(simulation estimate)`,
			subText: ko
	...
	return {
		...
		amount,
		mainText: ko
			? `월 약 ${formatted}원 광고비 방어 중 *(시뮬레이션 추정치)`
			: `Defending about ₩${formatted}/mo in paid-ad spend *(simulation estimate)`,
		subText: ko
