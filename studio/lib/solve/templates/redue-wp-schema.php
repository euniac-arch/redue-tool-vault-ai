...
if ( ! defined( 'REDUE_WP_SCHEMA_ENGINE' ) ) {
	define( 'REDUE_WP_SCHEMA_ENGINE', '1.2.0' );
}
...
$GLOBALS['redue_org_type']  = array('LocalBusiness', 'Organization');
$GLOBALS['redue_is_medical'] = false;
...
if ( ! function_exists( 'redue_wp_extract_footer_entities' ) ) {
	function redue_wp_extract_footer_entities( $html ) {
		...
		if ( preg_match( '/(?:사업자\\s*(?:등록)?\\s*번호|사업자번호|등록번호)\\s*[:：]?\\s*([0-9]{3}-[0-9]{2}-[0-9]{5})/u', $plain, $m ) ) {
			$out['taxID'] = trim( $m[1] );
		}
		if ( preg_match( '/(?:팩스|FAX|Fax|F\\.)\\s*[:：]?\\s*([0-9]{2,4}-[0-9]{3,4}-[0-9]{4})/u', $plain, $m ) ) {
			$out['faxNumber'] = trim( $m[1] );
		}
		if ( preg_match_all( '/(?:대표자|대표원장|원장|대표이사|대표)\\s*[:：]?\\s*([가-힣]{2,4})(?=\\s|<|$|\\||\\/)/u', $plain, $mm, PREG_SET_ORDER ) ) {
			$_stop = array( '인사말', '안내', '고객센터', '오시는길', '바로가기', '더보기', '자세히보기', '이사회', '이사', '제품으로', '제품', '대표', '문의', '상담', '진료', '정보' );
			foreach ( $mm as $_row ) {
		...
		if ( preg_match( '/(?:대표전화|전화번호|고객센터|TEL|Tel|T\\.)\\s*[:：]?\\s*([0-9]{2,4}-[0-9]{3,4}-[0-9]{4}|1[568]\\d{2}-\\d{4})/u', $plain, $m ) ) {
			$out['telephone'] = trim( $m[1] );
		}
		if ( preg_match( '/(?:주소|위치|소재지)?\\s*[:：]?\\s*([가-힣]+(?:특별시|광역시|도|시|군|구)\\s+[가-힣0-9\\s·\\-\\(\\),]+(?:로|길|동|리|가|번지|호|층|관|빌딩|호텔)[가-힣0-9\\s·\\-\\(\\),]*)/u', $plain, $m ) ) {
	...
	function redue_wp_is_medical( $org_types, $corpus = '' ) {
		...
		if ( is_array( $org_types ) ) {
			foreach ( $org_types as $_t ) {
				if ( in_array( $_t, array( 'MedicalClinic', 'Physician', 'Hospital', 'Dentist', 'VeterinaryCare', 'Pharmacy', 'MedicalBusiness' ), true ) ) {
					return true;
				}
			}
		}
