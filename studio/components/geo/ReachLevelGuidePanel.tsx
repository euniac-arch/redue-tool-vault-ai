...
export function ReachLevelGuideDetail({
	...
	return (
		<div
			className={`space-y-2.5 rounded-xl border p-3.5 ${tone.cardLight}`}
			aria-live="polite"
			...
			<div className="flex flex-wrap items-start justify-between gap-2">
				<div className="min-w-0">
					<div className="flex flex-wrap items-center gap-2">
						...
						<p className={`text-[12px] font-extrabold ${tone.titleLight}`}>{guide.title}</p>
						<span className={`inline-flex rounded-md px-1.5 py-0.5 text-[10px] font-extrabold ${tone.badgeLight}`}>
							{guide.badgeText}
						</span>
					</div>
					<p className="mt-1 text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">{guide.shortDesc}</p>
				</div>
			</div>

			<div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
				<div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 sm:col-span-2 dark:border-white/5 dark:bg-slate-950/50">
					<p className="mb-1 flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">
						<Compass className="h-3 w-3" aria-hidden />
						{t('reachMechanismLabel')}
					</p>
					<p className="text-[11px] leading-relaxed text-slate-700 dark:text-slate-200">{guide.reachMechanism}</p>
				</div>
				<div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 dark:border-white/5 dark:bg-slate-950/50">
					<p className="mb-1 flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">
						<Target className="h-3 w-3" aria-hidden />
						{t('exposureScopeLabel')}
					</p>
					<p className="text-[11px] leading-relaxed text-slate-700 dark:text-slate-200">{guide.exposureScope}</p>
				</div>
				<div className="rounded-lg border border-slate-200 bg-white px-2.5 py-2 dark:border-white/5 dark:bg-slate-950/50">
					<p className="mb-1 text-[10px] font-extrabold uppercase tracking-[0.12em] text-slate-500 dark:text-slate-400">
						{t('examplePatternLabel')}
					</p>
					<p className="font-mono text-[11px] leading-relaxed text-indigo-700 dark:text-indigo-200">{guide.examplePattern}</p>
				</div>
			</div>
		</div>
	);
