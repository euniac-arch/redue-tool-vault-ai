...
function matchesQuery(project: ProjectListItem, query: string): boolean {
	...
	const hay = `${projectDisplayName(project)} ${code} ${project.targetUrl} ${project.id}`.toLowerCase();
		...
		<div ref={rootRef} className="relative">
			<label className="flex flex-col gap-1.5">
				...
				<div className="relative">
					<input
						...
						className="w-full rounded-lg border border-slate-200 bg-white py-2.5 pl-3 pr-9 text-sm text-slate-800 outline-none ring-slate-900/10 focus:border-slate-400 focus:ring-2 disabled:bg-slate-50 disabled:text-slate-400 dark:bg-slate-800 dark:text-slate-100 dark:border-slate-700 dark:focus:border-slate-500"
					/>
					<span
						className="pointer-events-none absolute inset-y-0 right-2.5 flex items-center text-slate-400 dark:text-slate-500"
						aria-hidden
					>
			...
			{open ? (
				<ul
					id={listId}
					role="listbox"
					className="absolute z-30 mt-1 max-h-64 w-full overflow-y-auto rounded-lg border border-slate-200 bg-white py-1 shadow-lg dark:bg-slate-800 dark:border-slate-700"
				>
					{filtered.length === 0 ? (
								...
								<li key={project.id} role="option" aria-selected={active}>
									<button
										...
										className={`flex w-full flex-col gap-0.5 px-3 py-2 text-left transition-colors ${
											active ? 'bg-slate-900 text-white' : 'hover:bg-slate-50 dark:hover:bg-slate-700'
										}`}
									>
										<span
