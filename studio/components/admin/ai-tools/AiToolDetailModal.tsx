...
import { AiToolCategoryBadge, CATEGORY_SOLID_ACCENT, PricingTypeBadge } from './ai-tools-badges';
...
export function AiToolDetailModal({ tool, onClose }: AiToolDetailModalProps) {
	...
	return (
		...
		>
			<div
				...
				className="relative flex max-h-[92dvh] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl border border-slate-200 bg-white shadow-2xl sm:rounded-2xl dark:border-slate-700 dark:bg-slate-800"
			>
				<header className="flex items-start justify-between gap-3 border-b border-slate-200 px-5 py-4 dark:border-slate-700">
					<div className="flex min-w-0 items-center gap-3">
						...
						<div className="min-w-0">
							<h2 id="ai-tool-detail-title" className="truncate text-lg font-bold text-slate-900 dark:text-slate-100">
								{tool.name}
							</h2>
					...
					<div className="flex shrink-0 items-center gap-1.5">
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="hidden items-center gap-1.5 rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white transition hover:bg-slate-700 sm:inline-flex dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200"
						>
						...
						<button
							...
							onClick={onClose}
							aria-label="닫기"
							className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-slate-200 text-slate-500 hover:bg-slate-50 hover:text-slate-800 dark:border-slate-700 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-slate-100"
						>
							<X className="h-4 w-4" />
						</button>
					</div>
				...
				<div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
					<div className="flex flex-col gap-6">
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="inline-flex items-center justify-center gap-1.5 rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white transition hover:bg-slate-700 sm:hidden dark:bg-white dark:text-slate-950 dark:hover:bg-slate-200"
						>
							<ExternalLink className="h-3.5 w-3.5" aria-hidden />
							공식 사이트 바로가기
						</a>
						...
						<section>
							<SectionTitle icon={Wallet} label="요금제 & 플랜 정책" />
						...
						<section>
							<SectionTitle icon={CheckCircle2} label="핵심 사용 순서" />
							<ol className="mt-2.5 flex flex-col gap-2.5">
								{tool.guide.steps.map((step, index) => (
									<li key={index} className="flex items-start gap-2.5">
										<span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-slate-900 text-[10px] font-bold text-white dark:bg-white dark:text-slate-950">
											{index + 1}
										</span>
