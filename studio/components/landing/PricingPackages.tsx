...
const PLANS = [
	{
		...
		unit: 'text-slate-500',
		list: 'text-slate-300',
		cta: 'border border-slate-700 bg-slate-800 py-3 font-medium text-slate-100 hover:bg-slate-700',
	},
	{
		key: 'pro',
		href: '/contact',
		...
		card: 'relative overflow-hidden border-2 border-cyan-500/50 bg-gradient-to-b from-slate-900 via-slate-900 to-cyan-950/30 shadow-[0_0_30px_rgba(6,182,212,0.15)] md:-translate-y-2',
		name: 'text-xl font-black text-cyan-300',
		tagline: 'text-cyan-200/80',
		price: 'text-white',
		unit: 'text-slate-400',
		list: 'text-slate-200',
		cta: 'bg-gradient-to-r from-cyan-500 to-blue-600 py-3.5 font-bold text-white shadow-lg shadow-cyan-900/40 hover:from-cyan-400 hover:to-blue-500',
	},
	{
		key: 'enterprise',
		href: '/enterprise',
		items: 5,
		order: 'order-3',
		card: 'border border-slate-800/60 bg-slate-950/40 opacity-85',
		name: 'text-lg font-bold text-slate-300',
		tagline: 'text-slate-500',
		price: 'text-slate-300',
		unit: 'text-slate-500',
		list: 'text-slate-400',
		cta: 'border border-slate-800 bg-slate-900 py-3 font-medium text-slate-400 hover:bg-slate-800',
	},
] as const;
	...
	return (
		<section id="pricing" className="mt-20 scroll-mt-24 sm:mt-24">
			<div className="mx-auto w-full max-w-[960px]">
							...
							<article className={`flex h-full flex-col justify-between rounded-2xl p-6 ${plan.card}`}>
								<div>
									{plan.key === 'pro' ? (
										...
										<span className="inline-flex rounded border border-slate-700 bg-slate-800/60 px-2 py-0.5 text-[11px] font-medium text-slate-400">
											{t('enterpriseBadge')}
										</span>
									) : null}
				...
				<div className="mt-6 rounded-xl border border-slate-800 bg-slate-950/80 p-4 text-[11.5px] leading-relaxed text-slate-400">
					<p className="mb-1 font-semibold text-slate-300">{t('refundNotice.title')}</p>
