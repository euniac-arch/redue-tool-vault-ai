...
const SEGMENTS = [
	{ key: 'other', pct: 52, bar: 'bg-slate-600 hover:bg-slate-500', dot: 'bg-slate-500', value: 'text-slate-200' },
	{ key: 'a', pct: 27, bar: 'bg-amber-500 hover:bg-amber-400', dot: 'bg-amber-500', value: 'text-amber-300' },
	{ key: 'b', pct: 16, bar: 'bg-orange-600 hover:bg-orange-500', dot: 'bg-orange-500', value: 'text-orange-300' },
	{ key: 'own', pct: 5, bar: 'bg-rose-500 hover:bg-rose-400', dot: 'bg-rose-500', value: 'text-rose-400' },
] as const;
		...
		<section className="mt-20 sm:mt-24">
			<div className="mx-auto w-full max-w-[960px]">
				<div className="text-center">
					<p className="inline-flex rounded-full border border-violet-400/25 bg-violet-500/10 px-3 py-1 text-[11px] font-bold tracking-[0.16em] text-violet-300">
						{t('badge')}
					</p>
					<h2 className="mt-3">
						<span className="block text-lg font-medium text-slate-400 sm:text-xl">{t('title')}</span>
				...
				<div className="mt-8 rounded-2xl border border-slate-800 bg-[#0B1120] p-6 sm:p-8">
					<p className="text-xs uppercase tracking-wider text-slate-400">{t('barTitle')}</p>

					<div
						className="mt-3 flex h-4 overflow-hidden rounded-full border border-slate-800 bg-slate-950 p-0.5"
						role="img"
					...
					<ul className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
						{SEGMENTS.map(({ key, pct, dot, value }) => (
							<li
								key={key}
								className="rounded-xl border border-transparent bg-slate-900/60 p-3 transition-colors duration-200 hover:border-slate-700 hover:bg-slate-900"
							>
					...
					<div className="mt-6 rounded-xl border border-slate-800/80 bg-slate-950/70 p-5">
						<div className="flex flex-col items-center justify-between gap-6 sm:flex-row">
							...
							<div className="flex flex-col items-center gap-2">
								...
								<p className="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-3.5 py-1.5 text-xs font-bold text-cyan-300 shadow-sm">
									{t('lift')}
								</p>
							</div>
