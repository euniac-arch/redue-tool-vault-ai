...
export function WhyGeoCards() {
	...
	return (
		<div className="flex flex-col gap-5">
			<h2 className="text-center text-lg font-extrabold tracking-tight text-white sm:text-xl md:text-2xl">
			...
			<div className="grid w-full grid-cols-1 gap-3 text-left md:grid-cols-3 md:gap-4">
				{CARDS.map(({ key, emoji, iconWrap }) => (
					...
					>
						...
						<div>
							<p className="text-sm font-extrabold text-white sm:text-[15px]">{t(`whyCards.${key}.title`)}</p>
							<p className="mt-0.5 text-xs font-semibold text-accent-light">{t(`whyCards.${key}.subtitle`)}</p>
						</div>
