	...
	return (
		<section id="faq" className="mt-20 scroll-mt-24 sm:mt-24">
			<div className="mx-auto w-full max-w-[960px]">
				...
				<ul className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-[#0B1120] dark:shadow-none">
					{ITEMS.map((key) => {
						const expanded = open === key;
						return (
							<li key={key} className="border-b border-slate-200 last:border-b-0 dark:border-slate-800">
								<button
									type="button"
								...
								>
									<span className="break-keep text-[15px] font-bold leading-snug text-slate-800 dark:text-slate-100">
										{t(`items.${key}.q`)}
									</span>
									<ChevronDown
								...
								>
									<div className="overflow-hidden">
										<p className="break-keep px-5 pb-4 text-[13px] leading-relaxed text-slate-600 dark:text-slate-400">
											{t(`items.${key}.a`)}
										</p>
									</div>
								</div>
