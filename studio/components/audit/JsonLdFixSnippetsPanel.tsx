			...
			{snippets.length === 0 ? (
				...
				<div className="flex flex-col gap-4">
					{snippets.map((snippet) => (
						<div key={snippet.id} className="rounded-xl border border-white/[0.08] bg-black/20 p-4">
							<div className="flex flex-wrap items-start justify-between gap-2">
								<div>
									<p className="text-sm font-bold text-slate-100">{snippet.title}</p>
									<p className="mt-1 text-xs leading-relaxed text-slate-400">{snippet.description}</p>
								</div>
								<CopyButton code={snippet.code} copyLabel={t('copy')} copiedLabel={t('copied')} />
							</div>
							<pre className="mt-3 max-w-full overflow-x-auto whitespace-pre rounded-lg bg-black/40 px-3 py-2.5 font-mono text-[11px] leading-relaxed text-cyan-200/90">
								{snippet.code}
							</pre>
						</div>
					))}
