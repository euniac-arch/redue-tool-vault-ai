...
export function FloatingTabIndicator({
	...
	return (
		<div
			className="pointer-events-auto absolute bottom-full left-0 mb-3 hidden origin-bottom-left flex-col items-start gap-1.5 rounded-2xl border border-slate-200/80 bg-white/95 p-2 shadow-xl backdrop-blur-md transition-all duration-200 xl:flex dark:border-slate-800 dark:bg-slate-900/95"
			role="navigation"
			...
			{TAB_ITEMS.map((item) => {
				...
				return (
					...
					>
						...
						{isActive ? (
							<span className="ml-1 h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-400" aria-hidden />
						) : null}
					</button>
				);
			})}
