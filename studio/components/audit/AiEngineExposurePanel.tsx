...
import { AlertCircle, AlertTriangle, CheckCircle2, ChevronDown, Check, Copy, Info, Loader2, RefreshCw, Zap } from 'lucide-react';
...
import { resolveGeoWorkGuideModel, type GeoWorkGuideModel } from '@/lib/audit/geo-work-guide';
...
function CopyCodeButton({ code, label, copiedLabel }: { code: string; label: string; copiedLabel: string }) {
	...
	return (
		<button
			...
			className="inline-flex shrink-0 items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-bold text-indigo-600 transition-colors hover:bg-indigo-50 dark:text-indigo-300 dark:hover:bg-indigo-500/10"
		>
			{copied ? (
				<>
...
/**
 * Expandable "🔴 cause title" row. Clicking it reveals a dynamic fix guide
 * (💡 해결 방법 & 추천 조치) generated by `getAiFixGuide`, bound to the
 * currently diagnosed site's own brand / domain / URL / industry data —
 * never a hardcoded business name.
 */
function CauseFactorRow({
	factor,
	guideModel,
	lang,
}: {
	factor: EngineCauseFactor;
	guideModel: GeoWorkGuideModel;
	lang: AuditLang;
}) {
	const t = useTranslations('audit.aiEngines');
	const [open, setOpen] = useState(false);
	const guide = useMemo(() => getAiFixGuide(factor, guideModel, lang), [factor, guideModel, lang]);
	const panelId = `fix-guide-${factor.id}-${guideModel.domain || 'site'}`.replace(/[^a-zA-Z0-9_-]/g, '');

	return (
		<li className="min-w-0 rounded-lg">
			<button
				type="button"
			...
			>
				...
				<div className="min-w-0 flex-1">
					<span className="flex items-center gap-1">
						<p className="min-w-0 flex-1 text-[11px] font-extrabold break-keep text-slate-800 dark:text-slate-100">
							{factor.title}
						</p>
						<ChevronDown
							className={`h-3 w-3 shrink-0 text-slate-400 transition-transform duration-200 dark:text-slate-500 ${
								open ? 'rotate-180' : ''
							}`}
							aria-hidden
						/>
					</span>
					<p className="mt-0.5 text-xs leading-relaxed break-keep text-slate-600 dark:text-slate-400">
						{factor.detail}
					</p>
					<span className="mt-1 inline-block text-[10px] font-semibold text-indigo-600 dark:text-indigo-300">
						{open ? t('fixGuideToggleCollapse') : t('fixGuideToggleExpand')}
					</span>
				</div>
			</button>
			{open ? (
				<div
					id={panelId}
					className="mt-2 min-w-0 rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs text-slate-800 dark:border-slate-700 dark:bg-slate-800/60 dark:text-slate-200"
				>
					<p className="text-[10px] font-extrabold text-slate-500 dark:text-slate-400">{t('fixGuideTitle')}</p>
					<p className="mt-1.5 min-w-0 leading-relaxed break-keep">{guide.actionSummary}</p>
					{guide.detail ? (
						<p className="mt-1.5 min-w-0 leading-relaxed break-keep text-slate-600 dark:text-slate-400">
							{guide.detail}
						</p>
					) : null}
					{guide.checklist && guide.checklist.length > 0 ? (
						<div className="mt-2">
							<p className="text-[10px] font-bold text-slate-500 dark:text-slate-400">
								{t('fixGuideChecklistTitle')}
							</p>
							<ul className="mt-1 list-disc space-y-1 pl-4">
								{guide.checklist.map((item) => (
									<li key={item} className="min-w-0 leading-relaxed break-keep">
										{item}
									</li>
								))}
							</ul>
					...
					{guide.codeSnippet ? (
						<div className="mt-2 min-w-0 overflow-hidden rounded-md border border-slate-200 bg-white dark:border-slate-700 dark:bg-slate-950/60">
							<div className="flex min-w-0 items-center justify-between gap-2 border-b border-slate-200 bg-slate-100 px-2 py-1 dark:border-slate-700 dark:bg-slate-900/60">
								<span className="min-w-0 truncate text-[10px] font-bold text-slate-500 dark:text-slate-400">
									{guide.codeLabel}
								</span>
								<CopyCodeButton
									code={guide.codeSnippet}
									label={t('fixGuideCopyCode')}
									copiedLabel={t('fixGuideCopied')}
								/>
							</div>
							<pre className="max-w-full overflow-x-auto p-2 text-[10.5px] leading-relaxed whitespace-pre text-slate-700 dark:text-slate-300">
...
function EngineCard({
	...
	return (
		<div className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-white/[0.08] dark:bg-black/20">
			...
			<div className="mt-3 border-t border-slate-200/80 pt-3 dark:border-white/[0.06]">
				...
				{factors.length > 0 ? (
					<ul className="mt-3 flex min-w-0 flex-col gap-2.5">
						{factors.map((factor) => (
							<CauseFactorRow key={factor.id} factor={factor} guideModel={guideModel} lang={lang} />
						))}
					</ul>
				) : (
...
export function AiEngineExposurePanel({
	...
	/** Dynamic fix-guide model — bound to this report's own brand/domain/URL/industry (never hardcoded). */
	const guideModel = useMemo(() => resolveGeoWorkGuideModel(report, lang), [report, lang]);
