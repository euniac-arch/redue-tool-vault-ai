...
export function PortfolioCard({ item, onVerify }: PortfolioCardProps) {
	...
	return (
		<article className="flex flex-col justify-between overflow-hidden rounded-2xl border border-slate-800/80 bg-[#0B1120]/80 backdrop-blur-sm transition-all duration-200 hover:border-cyan-500/40 hover:bg-[#0E162B] hover:shadow-[0_8px_30px_rgba(6,182,212,0.08)] lg:flex-row">
			...
			<div className="flex flex-1 flex-col gap-3 p-5">
				...
				<div className="mt-auto flex flex-wrap gap-2 pt-2">
					<button
						...
						className="rounded-lg border border-slate-800 bg-slate-900 px-3 py-1.5 text-xs font-semibold text-slate-300 transition-colors hover:border-cyan-500 hover:bg-cyan-500 hover:text-slate-950"
					>
						실제 SEO 진단
					</button>
					<button
			...
			<aside className="flex w-full shrink-0 flex-col gap-3 border-t border-slate-800/80 bg-[#0E162B]/60 p-4 lg:w-[273px] lg:border-l lg:border-t-0">
				<div className="flex justify-end">
					<span className="rounded-md border border-cyan-500/20 bg-cyan-500/10 px-3 py-1 text-xs font-bold text-cyan-400">
						{item.cmsType}
					</span>
				</div>
				...
				<div className="mt-auto flex flex-col items-center justify-center gap-0.5 rounded-lg border border-cyan-500/20 bg-cyan-500/10 px-2.5 py-2 text-center">
					<span className="text-xs font-bold text-cyan-400">▲ +{formatScore(gain)}pt 상승</span>
